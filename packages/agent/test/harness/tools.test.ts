...
describe("AgentHarness tools", () => {
	...
	describe("bash", () => {
		...
		it("prepares command, cwd, and an explicit environment with the turn context", async () => {
			...
			getOrThrow(await env.writeFile("workspace/cwd-marker", "marker", BACKGROUND_CONTEXT));
			...
			const tool = createBashTool<typeof context>({
				...
				prepare: async (execution, turnContext, callContext) => {
					...
					execution.command += `\ntest -f cwd-marker && printf '%s:%s:%s:%s' "$prefix" "\${PI_BASH_PREPARE_INHERITED-}" "$PI_BASH_PREPARE_EXPLICIT" cwd`;
				},
			});
			...
			expect(textOutput(result)).toBe("ready::explicit:cwd");
		});
