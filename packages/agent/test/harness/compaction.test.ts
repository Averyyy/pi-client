...
): { faux: FauxProviderHandle; model: Model<Api> } {
...
describe("harness compaction", () => {
	...
	it("preserves a forced pi-server legacy-tree cut point", () => {
		...
		const marker: SessionTreeEntry = {
			type: "custom_message",
			id: "legacy-keep-marker",
			parentId: second.id,
			timestamp: new Date(3).toISOString(),
			customType: "pi:validation-hint",
			content: "keep this marker",
			display: false,
		};

		const preparation = getOrThrow(
			prepareLegacyCompaction([first, second, marker], DEFAULT_COMPACTION_SETTINGS, {
