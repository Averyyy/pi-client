...
describe("ChunkRequest", () => {
	...
	it("approves proxy warning responses and retries the pi-server request", async () => {
		const warningUrl =
			"http://114.114.114.114:9421/proxycontrolwarn/httpwarning_123.html?ori_url=aHR0cHM6Ly9waS55cmV2YS5hc2lh&uid=0";
		const capturedRequests: { url: string; method?: string; body?: RequestInit["body"] }[] = [];

		const mockFetch = vi.fn(async (url: string, init?: RequestInit) => {
			capturedRequests.push({ url, method: init?.method, body: init?.body });

			const originalRequestCount = capturedRequests.filter(
				(request) => request.url === "http://pi-server.test/api/session/init",
			).length;
			if (url === "http://pi-server.test/api/session/init" && originalRequestCount === 1) {
				return new Response("", {
					status: 302,
					headers: { Location: warningUrl },
				});
			}
			if (url === warningUrl) {
				return new Response(
			...
			if (url === "http://pi-server.test/api/session/init") {
				return new Response(JSON.stringify({ ok: true }), {
					status: 200,
					headers: { "Content-Type": "application/json" },
				});
		...
		const request = new ChunkRequest({ serverUrl: "http://pi-server.test", authToken: "token" });
		const response = await request.postJson("/api/session/init", { sessionId: "h-proxy-test" });

		expect(response.ok).toBe(true);
		expect(capturedRequests.map((request) => request.url)).toEqual([
			"http://pi-server.test/api/session/init",
			warningUrl,
			expect.stringMatching(/^http:\/\/114\.114\.114\.114:9421\/proxycontrolwarn\/check\?/),
			"http://pi-server.test/api/session/init",
		]);
		expect(capturedRequests[0].body).toBe(capturedRequests[3].body);
	});
});
