...
describe("AgentSession actionable boundaries", () => {
	...
	it("preserves boundary metadata when pi-server omits a failed assistant", async () => {
		...
		try {
				...
				extensionFactories: [
					(pi) => {
						pi.on("turn_end", (event) => {
							...
							return { entries: [{ type: "custom", customType: "after-failure", data: true }] };
						});
					},
				],
			});
			...
			expect(branch.some((entry) => entry.type === "custom" && entry.customType === "after-failure")).toBe(true);
			expect(branch.some((entry) => entry.type === "context_edit" && entry.replacement === null)).toBe(true);
			expect(
...
describe("durable length recovery", () => {
	...
	it("finishes retry bookkeeping when a retry receives a nonretryable error", async () => {
		...
		expect(harness.eventsOfType("auto_retry_end")).toContainEqual(
			expect.objectContaining({ success: false, attempt: 1, finalError: "insufficient_quota" }),
		);
	});
