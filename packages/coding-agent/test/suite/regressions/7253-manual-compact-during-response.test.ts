...
describe("issue #7253: manual compaction during an active response", () => {
	...
	it("persists the aborted response before running the requested manual compaction", async () => {
		...
		// Enable threshold compaction only after the second request starts so this
		// regression remains about the manual-compaction race, not the fork's
		// intentional pre-next-provider compaction after tool results.
