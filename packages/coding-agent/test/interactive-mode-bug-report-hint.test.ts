...
describe("InteractiveMode bug report hints", () => {
	...
	test("does not suggest reports for unknown errors that remain retryable", () => {
		const context = { suggestBugReport: vi.fn() };
	...
	test("suggests reports for deterministic provider failures", () => {
		const context = { suggestBugReport: vi.fn() };

		maybeSuggestBugReport.call(context, errorMessage("insufficient_quota"));
