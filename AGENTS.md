...
- Keep cancellation state in `AgentSession`: show `Aborting...` and ignore repeated interrupts until the old operation finishes. New steering/follow-up messages submitted during cancellation must queue immediately and resume through the shared session run lifecycle only after cancellation ends; do not await cancellation inside queueing callbacks or make the old abort promise wait for the new run. Clearing/dequeueing those messages must prevent their automatic continuation. A pi-server disconnect is not cancellation: await explicit remote cancellation acknowledgement before continuing, and retain unconfirmed cancellations and queued intent for an explicit retry on failure.
- Keep validation evidence in ordinary tool results; do not cache failed commands or modified paths for repeated next-turn reminders. Corrected commands can differ in arguments or working directory, and unrelated success cannot prove a failure resolved.
- Keep TUI transcript rendering separate from model context projection: preserve the full active branch in the durable session tree, but for long compacted sessions initialize the visible transcript from the latest compaction-aware context and lazy-load older active-branch entries in bounded batches; render persisted compaction entries only once, and clear the working indicator only after `agent_settled`, restoring it after transient compaction or retry indicators while the session is still streaming. Intra-turn compaction emits `willRetry: false` and does not fire another `turn_start`, so restore from `session.isStreaming`, not from `willRetry`.
...
- Transient provider context such as extension overlays must travel as `ephemeralMessages`/`contextOverlay` on `/api/stream`; it must not be converted into durable pending tree entries or trigger full-tree sync.
...
- `pi-server` stream requests should include a `runId`; the server journals only the final assistant message so a client can recover or replay a completed run after a stream disconnect without retaining all deltas. Poll the same run only after an event-stream has been established. HTTP/proxy failures (502/503/504/524) and fetch failures before the stream starts stay `provider_stream` so session auto-retry can run; do not poll `/runs/` in that case, because a down proxy would reclassify the original retryable error as `history_reconcile`. Classify recovery transport failures for interrupted streams outside `provider_stream`. Clear a session's run journals when deleting that session.
- Structure pi-server failures with phase metadata (`session_init`, `tree_sync`, `provider_stream`, `history_reconcile`) and a `retryable` flag. Session auto-retry may re-run the whole stream path for `provider_stream` errors and transient session_init/tree_sync outages, except usage/quota/balance. Do not auto-retry `history_reconcile` recovery-transport failures or history-divergence stops, which can hide an in-flight server run or overwrite local state.
...
- When upstream canonical session projection is merged into the fork, keep forced pre-request compaction tri-state: no trigger, successful compaction, or cancelled/failed compaction. A forced attempt must not silently fall through to a second regular compaction, and projected token checks must leave unsent boundary input intact.
...
1. **Update CHANGELOGs**: maintain each package's `[Unreleased]` section as part of the normal release work. The release process moves those entries into the released version and creates the next `[Unreleased]` section; it must not require a separate interactive changelog prompt or user confirmation before publishing.
