...
- A server `404` for an existing client pending run does not prove the provider never executed. Preserve the pending marker and fail closed with recovery instructions; never acknowledge it as `not_found` or automatically submit a replacement provider run.
